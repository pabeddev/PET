...
export const obtenerUsuarios = async (token) => {
    ...
    try{
        const response = await axiosInstance.get('/api/v3/admins/users/', config)
...
export const obtenerUsuariosColaboradores = async (token) => {
    ...
    try{
        const responseCll = await axiosInstance.get('/api/v3/admins/rescuers/', config)
...
export const peticionesColaborador = async (token) => {
    ...
    try{
        const responseRq = await axiosInstance.get('/api/v3/admins/requests/', config)
...
export const eliminarUsuario = async (token, deleteId) => {
    ...
    try{
        const responseDlt = await axiosInstance.delete(`/api/v3/admins/users/${deleteId}`,config)
...
export const aceptarPeticion = async (token, postId) => {
    ...
    try{
        const reponseAcp = await axiosInstance.post(`api/v3/admins/requests/${postId}?action=activate` , null, config)
...
export const rechazarPeticion = async (token, postId) => {
    ...
    try{
        const reponseRjt = await axiosInstance.post(`api/v3/admins/requests/${postId}?action=reject` , null, config)
